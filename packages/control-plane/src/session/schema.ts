...
export const MIGRATIONS: readonly SchemaMigration[] = [
  ...
  {
    // IDs 36-38 have already shipped in the fork. Keep the new upstream
    // migration append-only so existing Durable Objects do not skip it.
    id: 39,
  ...
  {
    // IDs 43-45 have already shipped in the fork. Append upstream's alarm
    // state migration so existing Durable Objects do not skip it.
    id: 46,
    description: "Persist session alarm scheduling state",
    run: SESSION_ALARM_STATE_TABLE_SQL,
  },
  {
    // Keep the upstream runtime-version migration append-only for the same reason.
    id: 47,
  ...
  {
    // Upstream numbered this 45; that id already shipped in the fork, so it is
    // appended here to keep migrations append-only for existing Durable Objects.
    id: 49,
