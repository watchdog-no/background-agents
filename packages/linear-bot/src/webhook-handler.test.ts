...
describe("handleAgentSessionEvent environment targets", () => {
  ...
  it("sends a created event's top-level prompt context to the session", async () => {
    ...
    const content = promptBody(fetchMock)?.content;
    expect(content).toContain('<user_content source="linear_prompt_context" author="linear">');
    expect(content).toContain("Use the parent issue's migration constraints.");
  });
