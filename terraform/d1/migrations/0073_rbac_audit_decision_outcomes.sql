...
CREATE TABLE authorization_audit_events_v3 (
  id TEXT PRIMARY KEY,
  ...
  operation_result TEXT NOT NULL CHECK (
    operation_result IN ('applied', 'no_op', 'denied', 'rejected')
  ),
  metadata_json TEXT NOT NULL CHECK (
    ...
    AND COALESCE(
      ...
      OR (
        json_extract(metadata_json, '$.schema') = 'authorization_decision.v1'
        AND json_type(metadata_json, '$.httpMethod') = 'text'
        AND json_type(metadata_json, '$.httpPath') = 'text'
        AND json_type(metadata_json, '$.httpStatus') = 'integer'
        AND json_type(metadata_json, '$.requirements') = 'array'
      ),
      0
    )
  )
...
INSERT INTO authorization_audit_events_v3 (
...
ALTER TABLE authorization_audit_events_v3 RENAME TO authorization_audit_events;
