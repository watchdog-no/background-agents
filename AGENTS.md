...
- **Watchdog sandbox provider**: `terraform/environments/production/sandbox.auto.tfvars` selects
  Modal and overrides `TF_VAR_*` provider values. Keep credentials in Actions secrets. The Daytona
  production cutover has been reverted; generic Daytona support remains available upstream.
...
- **D1 migrations**: `scripts/d1-migrate.sh` tracks applied migrations by full filename; the numeric
  prefix only sets apply order, so an upstream sync reusing a number is safe. New fork-local
